#!/usr/bin/env python3
"""Validate a testcase patch metadata file against its JSON Schema.

Usage:
    python scripts/validate_patch_metadata.py --file path/to/patch.json
    python scripts/validate_patch_metadata.py --file path/to/patch.yaml --out report.json
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate a testcase patch metadata file.")
    parser.add_argument("--file", required=True, help="Path to patch metadata JSON/YAML file")
    ...
    schema = load_schema("testcase_patch.schema.json", schemas_dir)
