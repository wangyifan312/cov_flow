...
This is **not** a documentation-only repository. It contains Python code, JSON schemas, CLI scripts, an MCP server, sample project data, skills, and tests.
...
- Phase 1: Sample MVP (schemas, sample data, MCP tools, tests)
...
**Phase 0 through Phase 6D are complete.** Phase 6+ (eval LLM execution, multi-project) require explicit user approval before any work begins.
...
- `mock_data/` (sample project data and pre-built indexes)
- `tests/` (tests for schemas, scripts, and tools)

### Phase 1 — Minimal Runnable Sample MVP (Done)
- All JSON schemas (`project_manifest`, `coverage_gap`, `scenario_card`, `testcase_patch`)
- Sample project data (`mock_data/<project>/project_manifest.yaml`, `coverage_gaps.json`, index files)
- Validation scripts (`validate_manifest.py`, `validate_coverage_gaps.py`, `generate_mock_index.py`)
- MCP server (`server.py`) with 7 tools:
...
- Tests for schemas, scripts, and MCP tools
- `make validate`, `make build-indexes`, `make test`, `make smoke-server` all work under sample MVP
...
- **Phase 2b**: 4 new MCP sim tools (`sim_run_targeted_test`, `sim_get_test_result`, `sim_search_log`, `cov_get_coverage_diff`), static patch check script, audit logging service, simulation data, 2 eval cases
...
- **Phase 2d**: Code coverage extension — 7 coverage types (functional, line, branch, condition, toggle, fsm, assert), unified schema with `anyOf` conditional required fields, type-aware MCP tools and diff computation, 12 new gaps (27 total), 4 new classifications (10 total), 35 new tests, 2 new eval cases (6 total)
...
- **Phase 5b**: Real simulation execution infrastructure — SimExecutor + SimLogParser + UrgRunner, real-only mode (mock mode removed), 521 tests
- **Phase 6A**: 8 quick-win MCP tools — 21 tools total, 556 tests
- **Phase 6B**: 2 RTL tools — rtl_get_source_snippet, rtl_trace_fanin, 23 tools, 608 tests
- **Phase 6C**: Sim history indexer + feedback schema + cov_get_hit_history — 24 tools, 676 tests
- **Phase 6D**: 2 final tools — tb_get_sequence_source_snippet + wave_check_condition (permanent stub) — 26 tools, 693 tests
- **Total**: 26 MCP tools, 15 skill reference documents, 6 eval cases, ruff 0, mypy 0, make accept clean
- Phase 5b adds real subprocess execution (VCS/URG) via SimExecutor with security boundaries (test name validation, path traversal rejection, shlex + shell=False, cwd lock, timeout)
...
- All sim tools execute real VCS subprocess via SimExecutor with security boundaries
...
- **WP-4 Targeted Testcase Generation** (Done): `tb_read_source` MCP tool (4 component types: sequence/test/base_test/env, SourceResolver security boundaries, max_lines=1000/max_bytes=64KB); generic testcase generation prompt (`docs/testcase_gen_generic_prompt.md`); axi2ahb GAP_0006 validation (wrap8 targeted sequence generated)
...
- **WP-2 Manifest Schema Extension** (Done): 6 new optional fields in `simulation` block (`urg_cmd_template`, `urg_binary`, `urg_timeout_seconds`, `timeout_seconds`, `results_root`, `vdb_dir_template`), 4 convenience properties in `lib/manifest.py`
- **WP-3 MCP Tool Upgrades** (Done): 4 sim tools (`sim_run_targeted_test`, `sim_get_test_result`, `sim_search_log`, `cov_get_coverage_diff`) execute via SimExecutor, `_sim_result_to_dict` helper for SimResult serialization
- **WP-4 CLI Script Upgrade** (Done): `scripts/sim_runner.py --dry-run` flag, test name/seed validation, SimExecutor pipeline execution, human-readable summary, exit codes (0=pass, 1=run fail, 2=compile fail)
...
### Phase 6 — Additional MCP Tools and Infrastructure (Done)
- **Phase 6A**: 8 quick-win MCP tools (`spec_get_section`, `reg_find_field`, `reg_search_by_description`, `reg_get_ral_path`, `rtl_get_instance_info`, `tb_find_sequence`, `tb_get_base_test_template`, `tb_find_config_knob`), 21 tools total, 556 tests
- **Phase 6B**: 2 RTL tools (`rtl_get_source_snippet`, `rtl_trace_fanin`), source-backed signal lookups, 23 tools total, 608 tests
- **Phase 6C**: Simulation history index (`build_sim_history_index.py`), feedback report schema (`feedback_report.schema.json`), `cov_get_hit_history` MCP tool, 24 tools total, 676 tests
- **Phase 6D**: 2 final tools (`tb_get_sequence_source_snippet` convenience wrapper, `wave_check_condition` permanent stub via StubVerdiAdapter), SV sequence files for dma_subsystem, 26 tools total, 693 tests, ruff 0, mypy 0
- **Total**: 26 MCP tools, 693 tests, ruff 0, mypy 0, make accept clean
...
- Create and modify Python code, JSON schemas, YAML manifests, CLI scripts, MCP server code, sample project data, indexes, skill markdown, tests, `README.md`, `Makefile`, `pyproject.toml`.
...
These rules are non-negotiable. Phase 2 stub implementations (dry-run, stub data, no real tool calls) are allowed; real tool integration beyond the URG parser, SourceResolver, TB Indexer, and SimExecutor is not.
...
- Run `make validate` after modifying schemas or sample manifests.
